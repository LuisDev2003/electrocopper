...
const $createReview = $("#fm-create-review") as HTMLFormElement;
const $reviewList = $("#review-list") as HTMLUListElement;
const $message = $("#message") as HTMLDivElement;
const $stars = $$("#stars label") as NodeListOf<HTMLLabelElement>;
...
const renderStarComponent = (stars: number) => {
  let response = "";
...
const reviewRender = async () => {
  const data = await findAll<Review[]>({ endpoint: reviewController });

  $reviewList.innerHTML = data.map(reviewComponent).join("");
};

const handleCreateReview = async (event: SubmitEvent) => {
